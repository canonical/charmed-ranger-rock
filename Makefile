...
DOCKER_PORT = 6080
DOCKER_ARGS = ranger-admin -g 'daemon off:' \; start ranger-admin
...
clean:
	multipass exec $(ROCK_DEV) -- bash -c 'docker ps -q -f "name=$(DOCKER_NAME)" && docker stop $(DOCKER_NAME) && docker rm $(DOCKER_NAME) && docker rmi charmed-ranger-rock:$(ROCK_VERSION)'
	multipass exec $(ROCK_DEV) -- bash -c 'rm $(REPO_FOLDER)/charmed-ranger-rock_$(ROCK_VERSION)-$(UBUNTU_VER)-edge_amd64.rock'
...
run:
	multipass exec $(ROCK_DEV) -- bash -c 'cd $(REPO_FOLDER) &&\
		sudo skopeo --insecure-policy copy oci-archive:charmed-ranger-rock_$(ROCK_VERSION)-$(UBUNTU_VER)-edge_amd64.rock docker-daemon:charmed-ranger-rock:$(ROCK_VERSION)'
	multipass exec $(ROCK_DEV) -- bash -c "cd $(REPO_FOLDER) &&\
                docker run -d --name $(DOCKER_NAME) -p $(DOCKER_PORT):$(DOCKER_PORT) charmed-ranger-rock:$(ROCK_VERSION) --args $(DOCKER_ARGS)"
	...
	echo "Ranger is up on $$IP_ADDRESS:$(DOCKER_PORT)"
